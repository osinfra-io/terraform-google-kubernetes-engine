...
provider "kubernetes" {
  cluster_ca_certificate = base64decode(
    local.setup.cluster_ca_certificate
  )
  host  = "https://${local.setup.cluster_endpoint}"
...
data "terraform_remote_state" "setup" {
  backend   = "gcs"
  workspace = "kitchen-terraform-default-kubernetes-engine-gcp"
    ...
    template {
      ...
      spec {
        container {
          ...
          command = [
            "/bin/bash", "-c", "curl -H 'Metadata-Flavor: Google' http://169.254.169.254/computeMetadata/v1/instance/service-accounts/default/email | grep ${each.key}-k8s-wif@${var.project_id}.iam.gserviceaccount.com", "[ $? -ne 0 ] || exit 1"
          ]
